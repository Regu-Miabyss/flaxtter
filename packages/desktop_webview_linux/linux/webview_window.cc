...
WebviewWindow::WebviewWindow(FlMethodChannel *method_channel, int64_t window_id,
  ...
  g_signal_connect(G_OBJECT(window_), "destroy",
                   G_CALLBACK(+[](GtkWidget *, gpointer arg) {
                     ...
                     // GTK has already destroyed child widgets; avoid UAF in
                     // ~WebviewWindow. Defer map erase so we are not destroyed
                     // from inside this signal handler.
                     window->webview_ = nullptr;
                     window->title_bar_ = nullptr;
  ...
  // Optional Flutter title bar. Skip when height <= 0 to avoid spawning a
  // second Flutter engine in-process (EGL conflicts with the main app).
  if (title_bar_height > 0) {
    ...
    title_bar_ = GTK_WIDGET(fl_view_new(project));

    g_autoptr(FlPluginRegistrar) desktop_webview_linux_registrar =
        fl_plugin_registry_get_registrar_for_plugin(
            FL_PLUGIN_REGISTRY(title_bar_), "DesktopWebviewLinuxPlugin");
    client_message_channel_plugin_register_with_registrar(
        desktop_webview_linux_registrar);

    gtk_widget_set_size_request(title_bar_, -1, title_bar_height);
    gtk_widget_set_vexpand(title_bar_, FALSE);
    gtk_box_pack_start(box_, title_bar_, FALSE, FALSE, 0);
  }
...
WebviewWindow::~WebviewWindow() {
  if (webview_ != nullptr && G_IS_OBJECT(webview_)) {
...
void WebviewWindow::Close() {
  ...
  if (webview_ != nullptr && WEBKIT_IS_WEB_VIEW(webview_)) {
...
FlValue *WebviewWindow::GetAllCookies() {
  ...
  for (GList *l = cookies; l; l = l->next) {
    ...
    if (session_only) {
      ...
      const gint64 expires = g_date_time_to_unix(
          soup_cookie_get_expires(cookie));
#else
      const gint64 expires = static_cast<gint64>(
          soup_date_to_time_t(soup_cookie_get_expires(cookie)));
#endif
      fl_value_set_string_take(cookie_map, "expires",
                               fl_value_new_int(expires));
    }
...
struct ClearWebsiteDataRequest {
  FlMethodCall *call;
};

void on_clear_website_data(WebKitWebsiteDataManager *manager,
                           GAsyncResult *result, gpointer user_data) {
  auto *request = static_cast<ClearWebsiteDataRequest *>(user_data);
  GError *error = nullptr;
  webkit_website_data_manager_clear_finish(manager, result, &error);
  if (error != nullptr) {
    fl_method_call_respond_error(request->call, "clear_website_data_failed",
                                 error->message, nullptr, nullptr);
    g_error_free(error);
  } else {
...
void WebviewWindow::ClearWebsiteData(FlMethodCall *call) {
  if (webview_ == nullptr || !WEBKIT_IS_WEB_VIEW(webview_)) {
    fl_method_call_respond_error(call, "clear_website_data_failed",
                                 "webview is not available", nullptr, nullptr);
    return;
  }
  ...
  auto *request = new ClearWebsiteDataRequest{
      FL_METHOD_CALL(g_object_ref(call))};

  webkit_website_data_manager_clear(
      website_data_manager, WEBKIT_WEBSITE_DATA_ALL, 0, nullptr,
      (GAsyncReadyCallback)on_clear_website_data, request);
}
