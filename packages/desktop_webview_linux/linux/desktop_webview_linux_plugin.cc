...
struct DeferredWindowCleanup {
  DesktopWebviewLinuxPlugin *plugin;
  int64_t window_id;
};

gboolean deferred_window_cleanup(gpointer user_data) {
  auto *data = static_cast<DeferredWindowCleanup *>(user_data);
  data->plugin->windows->erase(data->window_id);
...
static void webview_window_plugin_handle_method_call(
  ...
  if (strcmp(method, "create") == 0) {
    ...
    auto webview = std::make_unique<WebviewWindow>(
        ...
        [self, window_id]() {
          auto *data = new DeferredWindowCleanup{self, window_id};
          g_idle_add(deferred_window_cleanup, data);
        },
    ...
    fl_method_call_respond_success(method_call, nullptr, nullptr);
  } else if (strcmp(method, "closeAll") == 0) {
    for (const auto &item : *self->windows) {
      item.second->Close();
    }
    fl_method_call_respond_success(method_call, nullptr, nullptr);
  } else if (strcmp(method, "clearWebsiteData") == 0) {
    auto *args = fl_method_call_get_args(method_call);
    if (fl_value_get_type(args) != FL_VALUE_TYPE_MAP) {
      fl_method_call_respond_error(method_call, "0",
                                   "clearWebsiteData args is not map", nullptr,
                                   nullptr);
      return;
    }
    auto window_id = fl_value_get_int(fl_value_lookup_string(args, "viewId"));
    ...
    self->windows->at(window_id)->ClearWebsiteData(method_call);
  } else if (strcmp(method, "clearAll") == 0) {
    // Deprecated: close windows only. Use clearWebsiteData on a live webview.
    for (const auto &item : *self->windows) {
      item.second->Close();
    }
    fl_method_call_respond_success(method_call, nullptr, nullptr);
